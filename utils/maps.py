...
def extract_keywords_from_query(query: str) -> str:
    ...
    food_keywords = ['food', 'eat', 'restaurant', 'cafe', 'coffee', 'pizza', 
                     'burger', 'sushi', 'mexican', 'italian', 'chinese', 'thai',
                     'breakfast', 'lunch', 'dinner', 'brunch', 'dessert', 'bakery']
    
    # Common attraction keywords
    attraction_keywords = ['attraction', 'visit', 'see', 'tour', 'museum', 
                          'park', 'landmark', 'monument', 'historic']
...
def get_nearby_places(query: str, latitude: float, longitude: float, radius: int = 1000) -> list:
    """
    ...
    try:
        ...
        for place in places_result['results'][:10]:  # Top 10 results
            ...
            if 'opening_hours' in place:
                enhanced_place['open_now'] = place['opening_hours'].get('open_now', False)
            else:
...
if __name__ == "__main__":
    ...
    if places:
        ...
        for i, place in enumerate(places[:3], 1):
            print(f"    {i}. {place['name']} ({place['rating']}★, {place['distance_km']}km)")
    else:
